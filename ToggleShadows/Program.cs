...
namespace FixGraphicsEngine
{
    class Program
    {
        static void Main()
        {
            ...
            string Contains = "The Shadows line is there. Continuing.";
            string DoesNotContain = "The Shadows line is not there. Writing line and continuing.";


            //Checks to see if the line is actually there in configs.
            if (text.Contains("07gfxsets_shadows::-%(|::"))
            {
            ...
            else
            {
                ...
                string billybob = Environment.NewLine + "07gfxsets_shadows::-%(|::1";
                System.IO.File.AppendAllText(fileName, billybob);
                NotBarf();
            }

            //Toggles the config.
            if (text.Contains("07gfxsets_shadows::-%(|::1"))
            {
                text = text.Replace("07gfxsets_shadows::-%(|::1", "07gfxsets_shadows::-%(|::0");
                File.WriteAllText(fileName, text);
                Barf();
            }
            if (text.Contains("07gfxsets_shadows::-%(|::0"))
            {
                text = text.Replace("07gfxsets_shadows::-%(|::0", "07gfxsets_shadows::-%(|::1");
                File.WriteAllText(fileName, text);
                NotBarf();
            }
            File.WriteAllText(fileName, text);
        ...
        static void Barf()
        {
            Console.WriteLine("Done! Set Shadows to false! Hit any key to quit!");
            Console.ReadKey();
        ...
        static void NotBarf()
        {
            Console.WriteLine("Done! Set Shadows to true! Hit any key to quit!");
            Console.ReadKey();
            Process.GetCurrentProcess().Kill();
        }
        static void UhOh()
